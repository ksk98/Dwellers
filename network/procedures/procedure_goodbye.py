...
def carry_out(sckt: socket.socket) -> str:
    ...
    if not context.GAME.remove_from_lobby(pid):
        return utility.get_ip_and_address_of_client_socket(sckt) + "PLAYER OF ID " + pid + " COULD NOT BE REMOVED"
    return utility.get_ip_and_address_of_client_socket(sckt) + " PLAYER OF ID " + pid + " REMOVED"
